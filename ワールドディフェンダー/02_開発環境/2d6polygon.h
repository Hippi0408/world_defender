...
//
//=============================================================================
#ifndef _2D6POLGON_H_			//このマクロ定義がされなかったら
#define _2D6POLGON_H_			//2重インクルード防止のマクロ定義
...
// クラス定義
//*****************************************************************************
class C2D6Polygon : public CObject
{
private:
	static const int SURFACE_NUMBER = 3;
	// 頂点フォーマット
	static const DWORD FVF_VERTEX_2D = (D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_TEX1);
	static const float FLASHING_AMOUNT;

public:
	C2D6Polygon();//コンストラクタFlashing
	virtual ~C2D6Polygon() override;//デストラクタ
	...
	void SetColor(D3DXCOLOR col,int nNum) { m_Col[nNum] = col; }
	void AddColor(D3DXCOLOR col, int nNum) { m_Col[nNum] += col; }
	D3DXCOLOR GetColor(int nNum) { return m_Col[nNum]; }
	...
	void AddRatio(float fRatio) { m_fRatio += fRatio;}

private:
	LPDIRECT3DVERTEXBUFFER9 m_pVtxBuff;//頂点バッファへのポインタ
	D3DXVECTOR3 m_Pos;//位置
	D3DXVECTOR3 m_AddMiddle;//位置
	D3DXVECTOR3 m_Rot;//向き
	D3DXVECTOR3 m_Move;//移動量
	D3DXVECTOR2 m_UV;//UVの座標
	D3DXVECTOR2 m_UVSize;//UVのサイズ

	D3DXCOLOR m_Col[SURFACE_NUMBER];//色
